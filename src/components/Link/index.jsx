...
const Link = ({ titlelink, linkin }) => {
    return (
        <div className="flex flex-wrap text-xs">
            {
                titlelink.map((e, i) => (
                    <a key={`${e}-${i}`} href={linkin[i]} target="_blank" rel="noopener noreferrer" className="hover:text-primarybtn transition-all text-primaryContent text-sm px-1.5 py-1 rounded-full mr-1.5 mt-2"><LinkIcon /> {e}</a>
                ))
            }
        </div>
    )
